...
function buildPrompt(rawText: string) {
  return [
    ...
    "OCR может быть кривым: буквы и цифры могут слипаться, таблица может быть сломана, колонки могут поехать.",
    "Название товара обычно в начале строки. Количество, единица, цена и сумма могут стоять рядом без идеального разделения.",
    "Если видишь строку-кандидат товара, всё равно верни её в items, даже если уверен не полностью.",
    "Если не хватает числовых полей, оставь их null, но строку товара не выбрасывай.",
    ...
    "Лучше вернуть несколько строк-кандидатов товаров с null-полями, чем пустой items.",
    "Схема ответа:",
    JSON.stringify({
      supplierName: null,
      invoiceNumber: null,
      ...
      messages: [
        {
          role: "system",
          content:
            "Ты аккуратно разбираешь русские накладные в JSON. Возвращай только факты из текста, без догадок. Если OCR грязный, всё равно старайся выделить строки товаров-кандидатов и не возвращай пустой items без крайней необходимости.",
        },
        {
          role: "user",
